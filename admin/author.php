...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $search = $_POST["search"];
    $sql = "SELECT author.id, author.author_name, GROUP_CONCAT(books.book_title SEPARATOR ', ') AS book_title 
            FROM author 
            LEFT JOIN books ON author.id = books.author_id 
            WHERE author.author_name LIKE '%$search%' OR books.book_title LIKE '%$search%'
            GROUP BY author.id
...
<body>
    ...
    <div class="title">Manage Author's
    <a href="insert-author.php">
    <button class="add-btn"><i class='bx bxs-plus-circle'></i>Add Author</button>
    </a>
        ...
        <div class="search-bar">
                <form method="post" action="">
                    ...
                    <input type="text" id="search-info" name="search" placeholder="&nbsp; &nbsp; &nbsp; Search for author name & books written" class="left-align input-field">
                </form>
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            echo '<td>' . ($row['book_title'] ? $row['book_title'] : '--') . '</td>'; // Display "--" if book_title is empty
            echo '<td>'; // Action column
            echo '<a href="edit-author.php?id=' . $row['id'] . '"><i class="bx bx-edit"></i></a>'; // Edit button
            echo '<a href="delete-author.php?id=' . $row['id'] . '" onclick="return confirm(\'Are you sure you want to delete this record?\');"><i class="bx bx-trash"></i></a>'; // Delete button
            echo '</td>';
            echo '</tr>';
        }
        echo '</table>';
