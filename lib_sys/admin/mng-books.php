...
// Get records from the database based on search input
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $search = $_POST["search"];
    $sql = "SELECT books.id, books.qty, books.image, books.isbn, books.book_title, tag.tag_name, author.author_name, publisher.publisher_name FROM books
    INNER JOIN tag ON books.tag_id = tag.id
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
$search = $_POST["search"];
$sql = "SELECT books.id, books.qty, books.isbn, books.book_title, tag.tag_name AS tag_name, author.author_name AS author_name, publisher.publisher_name AS publisher_name
FROM books
INNER JOIN tag ON books.tag_id = tag.id
INNER JOIN author ON books.author_id = author.id
INNER JOIN publisher ON books.publisher_id = publisher.id
WHERE books.isbn LIKE '%$search%' OR books.book_title LIKE '%$search%' OR tag.tag_name LIKE '%$search%' OR author.author_name LIKE '%$search%' OR publisher.publisher_name LIKE '%$search%'
...
<body>
    ...
    <div class="title">Manage Book's       
        </div>

        <div class="search-bar">
                <form method="post" action="">
                    <i class="lni lni-magnifier" style="position: absolute; color: gray; padding-left: 90px; padding-top: 12px; left: 0px;"></i>
                    <input type="text" id="search-info" name="search" placeholder="&nbsp; &nbsp; &nbsp; Search for ISBN, title, author, and genre" class="left-align input-field">
                </form>
        ...
        while ($row = mysqli_fetch_array($result)) {
            ...
            echo '<a href="edit-book.php?id=' . $row['id'] . '"><i class="bx bx-edit"></i></a>'; // Edit button
            echo '<a href="delete-book.php?id=' . $row['id'] . '" onclick="return confirm(\'Are you sure you want to delete this record?\');"><i class="bx bx-trash"></i></a>'; // Delete button
            echo '</td>';
            echo '</tr>';
        }
        echo '</table>';
